...
# %%
V=[]
V1=[i for i in range(14,81)]
V2=[i for i in range(81,148)]
V3=[i for i in range(220,287)]

# %%
VV1=df7.loc[0][V1].keys()
VV2=df7.loc[0][V2].keys()
VV3=df7.loc[0][V3].keys()
VV=VV1.append(VV2.append(VV3))

# %%
DF71=df7.copy()
T=[i for i in range(148,215)]
T1=list(df7.loc[0][T].keys())
A=[]
for y in VV:
    if (DF71[y] == 0).all():
        A.append(y)
#DF71=DF71.drop(A,axis=1)   
...
if F=='Y' or F=='Y'.lower():
    ...
    if G=='Y' or G=='Y'.lower():
        DF73=DF72.drop(A,axis=1)
        writer = pd.ExcelWriter(Name+'.xlsx')
        DF73.to_excel(writer,Name)
        writer.close()
    if G=='N' or G=='N'.lower():
        writer = pd.ExcelWriter(Name+'.xlsx')
        DF72.to_excel(writer,Name)
        writer.close()
...
# %%
print ('Votos a candidaturas <3%:',f"{df0[New_l].sum(axis=1).sum()-df7[l].sum(axis=1).sum():,.0f}")
